...
class VideoDataset(Dataset):
    ...
    def __init__(self, json_path: str, video_dir: str, num_frames: int = 12):
        """
        Args:
    ...
    def sample_frames(self, video_path: str) -> List[Image.Image]:
        """
...
def plot_training_loss(loss_history, epoch_losses, output_dir, training_type="LoRA", eval_losses=None, eval_metrics=None):
    num_plots = 3 if eval_losses is not None else 2
    ...
    if eval_losses is not None:
        axes[ax_idx].plot(range(1, len(eval_losses) + 1), eval_losses,
                         label=f'{training_type} Eval Loss', marker='s', linewidth=2)
        axes[ax_idx].set_xlabel('Epoch')
        axes[ax_idx].set_ylabel('Eval Loss')
        axes[ax_idx].set_title(f'{training_type} Evaluation Loss per Epoch')
        axes[ax_idx].grid(True, alpha=0.3)
        axes[ax_idx].legend()
        ax_idx += 1
...
def evaluate_video(
    ...
    """
    Evaluate video dataset (compute loss and CIDEr/ROUGE-L/METEOR/BLEU)
    ...
    with torch.no_grad():
        for idx in eval_pbar:
            ...
            ground_truth_for_loss = random.choice(captions)
            frames = eval_dataset.sample_frames(video_path)
            
            memory = MemoryBank(max_history=max_history)
            raw_w = [(t + 1) / num_frames for t in range(num_frames)]
            sum_w = sum(raw_w)
            norm_weights_eval = [w / sum_w for w in raw_w]
            video_loss_accum = torch.tensor(0.0, device=device)
            final_prediction = ""
            ...
            for t in range(num_frames):
                ...
                prompt = build_prompt_with_context(context_texts)
                ground_truth = ground_truth_for_loss
                ...
                outputs = model(**inputs, labels=labels)
                video_loss_accum = video_loss_accum + outputs.loss * norm_weights_eval[t]
                ...
                del outputs, inputs, labels, generated_ids, generated_ids_trimmed, inputs_gen

                if t == num_frames - 1:
                    final_prediction = prediction_t

            eval_loss_sum += video_loss_accum.item()
            eval_steps += 1
            ...
            if accelerator.is_local_main_process:
                ...
                eval_pbar.set_postfix({'loss': f'{video_loss_accum.item():.4f}', 'avg': f'{current_avg_loss:.4f}'})
...
def train(
    ...
    processor = AutoProcessor.from_pretrained(
        "./model", min_pixels=128*28*28, max_pixels=256*28*28, padding_side="right"
    )
    
    train_dataset = VideoDataset(train_json_path, video_dir, num_frames=num_frames)
    eval_dataset = VideoDataset(eval_json_path, video_dir, num_frames=num_frames)
    test_dataset = VideoDataset(test_json_path, video_dir, num_frames=num_frames)
    
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True, pin_memory=True, num_workers=2)
    eval_loader = DataLoader(eval_dataset, batch_size=1, shuffle=False, pin_memory=True, num_workers=2)
    ...
    try:
        for epoch in range(epochs):
            ...
            for batch in train_pbar:
                ...
                video_path = os.path.join(video_dir, batch['video'][0])
                captions = batch['caption'][0]
                # Ensure captions is a list
                if isinstance(captions, str):
                    captions = [captions]
                
                frames = train_dataset.sample_frames(video_path)
            ...
            eval_loss, metrics = evaluate_video(
            ...
            if accelerator.is_local_main_process and epoch_steps > 0:
                ...
                logger.info(f"   Evaluation average Loss: {eval_loss:.6f}")
                logger.info("=" * 80)
        ...
        if accelerator.is_local_main_process and len(loss_history) > 0:
            ...
            plot_training_loss(
                ...
                eval_losses=eval_losses, eval_metrics=eval_metrics_history
            )
    ...
    final_test_loss, final_test_metrics = evaluate_video(
    ...
    if accelerator.is_local_main_process:
        final_results = {
            ...
            'training_config': {
                ...
                'best_eval_loss': early_stopper.best_loss,
            }
        }
        ...
        if writer is not None:
            writer.add_scalar('Test/final_loss', final_test_loss, 0)
